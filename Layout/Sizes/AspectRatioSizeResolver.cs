    ...
    public class AspectRatioSizeResolver : IKeyboardSizeResolver
    {
        ...
        public (Rect, int) ResolveSize(Size sourceSize)
        {
            if (sourceSize.Width > 0 && sourceSize.Height > 0)
            {
                ...
                var keyGap = (int) Math.Max(Math.Min(diameter / 300, 5), 0) + 1;

                var offsetX = ((float) sourceSize.Width - destinationWidth) / 2f;
                var offsetY = ((float) sourceSize.Height - destinationHeight) / 2f;
